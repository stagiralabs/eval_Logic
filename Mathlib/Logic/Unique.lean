...
@[target] theorem unique_iff_existsUnique (α : Sort u) : Nonempty (Unique α) ↔ ∃! _ : α, True := by sorry

@[deprecated (since := "2024-12-17")] alias unique_iff_exists_unique := unique_iff_existsUnique

@[target] theorem unique_subtype_iff_existsUnique {α} (p : α → Prop) :
    Nonempty (Unique (Subtype p)) ↔ ∃! a, p a := by sorry
...
@[target] theorem PUnit.default_eq_unit : (default : PUnit) = PUnit.unit := by sorry
...
@[target] theorem eq_default (a : α) : a = default := by sorry
...
@[target] theorem exists_iff {p : α → Prop} : Exists p ↔ p default := by sorry
...
@[target] theorem nonempty_unique (α : Sort u) [Subsingleton α] [Nonempty α] : Nonempty (Unique α) := by sorry
...
@[target] theorem Pi.default_def {β : α → Sort v} [∀ a, Inhabited (β a)] :
    @default (∀ a, β a) _ = fun a : α ↦ @default (β a) _ := by sorry
...
@[target] theorem eq_const_of_subsingleton {β : Sort*} [Subsingleton α] (f : α → β) (a : α) :
    f = Function.const α (f a) := by sorry

@[target] theorem eq_const_of_unique {β : Sort*} [Unique α] (f : α → β) : f = Function.const α (f default) := by sorry
...
@[target] theorem uniqueElim_const {β : Sort*} {_ : Unique ι} (x : β) (i : ι) :
    uniqueElim (α := by sorry
...
@[target] theorem subsingleton_iff_isEmpty {α : Type u} : Subsingleton (Option α) ↔ IsEmpty α := by sorry
