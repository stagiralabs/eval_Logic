...
@[target] theorem nontrivial_iff : Nontrivial α ↔ ∃ x y : α, x ≠ y := by sorry

@[target] theorem exists_pair_ne (α : Type*) [Nontrivial α] : ∃ x y : α, x ≠ y := by sorry

@[target] theorem exists_ne [Nontrivial α] (x : α) : ∃ y, y ≠ x := by sorry

@[target] theorem nontrivial_of_ne (x y : α) (h : x ≠ y) : Nontrivial α := by sorry

@[target] theorem nontrivial_iff_exists_ne (x : α) : Nontrivial α ↔ ∃ y, y ≠ x := by sorry
...
@[target] theorem subsingleton_iff : Subsingleton α ↔ ∀ x y : α, x = y := by sorry
...
@[target] theorem subsingleton_or_nontrivial (α : Type*) : Subsingleton α ∨ Nontrivial α := by sorry
