...
@[target] lemma eq_fiber_image  (f : Y → Z) (a : Fiber f) : a.1 = f ⁻¹' {a.image} := by sorry
...
@[target] lemma map_eq_image (f : Y → Z) (a : Fiber f) (x : a.1) : f x = a.image := by sorry
...
@[target] lemma map_preimage_eq_image (f : Y → Z) (a : Fiber f) : f a.preimage = a.image := by sorry

@[target] lemma fiber_nonempty (f : Y → Z) (a : Fiber f) : Set.Nonempty a.val := by sorry

@[target] lemma map_preimage_eq_image_map {W : Type*} (f : Y → Z) (g : Z → W) (a : Fiber (g ∘ f)) :
    g (f a.preimage) = a.image := by sorry

@[target] lemma image_eq_image_mk (f : Y → Z) (g : X → Y) (a : Fiber (f ∘ g)) :
    a.image = (Fiber.mk f (g (a.preimage _))).image := by sorry
