...
@[target] theorem encode_injective [Encodable α] : Function.Injective (@encode α _)
  | x, y, e => Option.some.inj <| by rw [← encodek, e, encodek] := by sorry

@[target] theorem encode_inj [Encodable α] {a b : α} : encode a = encode b ↔ a = b := by sorry
...
@[target] theorem encode_ofEquiv {α β} [Encodable α] (e : β ≃ α) (b : β) :
    @encode _ (ofEquiv _ e) b = encode (e b) := by sorry

@[target] theorem decode_ofEquiv {α β} [Encodable α] (e : β ≃ α) (n : ℕ) :
    @decode _ (ofEquiv _ e) n = (decode n).map e.symm := by sorry

instance _root_.Nat.encodable : Encodable ℕ :=
  ⟨id, some, fun _ => rfl⟩

@[target] theorem encode_nat (n : ℕ) : encode n = n := by sorry
...
@[target] theorem encode_star : encode PUnit.unit = 0 := by sorry

@[target] theorem decode_unit_zero : decode 0 = some PUnit.unit := by sorry

@[target] theorem decode_unit_succ (n) : decode (succ n) = (none : Option PUnit) := by sorry
...
@[target] theorem encode_none [Encodable α] : encode (@none α) = 0 := by sorry

@[target] theorem encode_some [Encodable α] (a : α) : encode (some a) = succ (encode a) := by sorry

@[simp]
theorem decode_option_zero [Encodable α] : (decode 0 : Option (Option α))= some none :=
  rfl

@[target] theorem decode_option_succ [Encodable α] (n) :
    (decode (succ n) : Option (Option α)) = (decode n).map some := by sorry

def decode₂ (α) [Encodable α] (n : ℕ) : Option α :=
  (decode n).bind (Option.guard fun a => encode a = n)

@[target] theorem mem_decode₂' [Encodable α] {n : ℕ} {a : α} :
    a ∈ decode₂ α n ↔ a ∈ decode n ∧ encode a = n := by sorry

theorem mem_decode₂ [Encodable α] {n : ℕ} {a : α} : a ∈ decode₂ α n ↔ encode a = n :=
  mem_decode₂'.trans (and_iff_right_of_imp fun e => e ▸ encodek _)

@[target] theorem decode₂_eq_some [Encodable α] {n : ℕ} {a : α} : decode₂ α n = some a ↔ encode a = n := by sorry

@[target] theorem decode₂_encode [Encodable α] (a : α) : decode₂ α (encode a) = some a := by sorry
...
@[target] theorem decode₂_is_partial_inv [Encodable α] : IsPartialInv encode (decode₂ α) := by sorry

@[target] theorem decode₂_inj [Encodable α] {n : ℕ} {a₁ a₂ : α} (h₁ : a₁ ∈ decode₂ α n)
    (h₂ : a₂ ∈ decode₂ α n) : a₁ = a₂ := by sorry

@[target] theorem encodek₂ [Encodable α] (a : α) : decode₂ α (encode a) = some a := by sorry
...
@[target] theorem encode_inl (a : α) : @encode (α ⊕ β) _ (Sum.inl a) = 2 * (encode a) := by sorry
...
@[target] theorem encode_true : encode true = 1 := by sorry

@[target] theorem encode_false : encode false = 0 := by sorry

@[simp]
theorem decode_zero : (decode 0 : Option Bool) = some false :=
  rfl

@[target] theorem decode_one : (decode 1 : Option Bool) = some true := by sorry
...
@[target] theorem decode_sigma_val (n : ℕ) :
    (decode n : Option (Sigma γ)) =
      (decode n.unpair.1).bind fun a => (decode n.unpair.2).map <| Sigma.mk a := by sorry

@[target] theorem encode_sigma_val (a b) : @encode (Sigma γ) _ ⟨a, b⟩ = pair (encode a) (encode b) := by sorry
...
@[target] theorem decode_prod_val (n : ℕ) :
    (@decode (α × β) _ n : Option (α × β))
      = (decode n.unpair.1).bind fun a => (decode n.unpair.2).map <| Prod.mk a := by sorry
...
@[target] theorem nonempty_encodable : Nonempty (Encodable α) ↔ Countable α := by sorry
...
@[target] theorem down_up {a : ULower α} : down a.up = a := by sorry

@[simp]
theorem up_down {a : α} : (down a).up = a := by
  simp [up, down,Equiv.left_inv _ _, Equiv.symm_apply_apply]

@[target] theorem up_eq_up {a b : ULower α} : a.up = b.up ↔ a = b := by sorry

@[target] theorem down_eq_down {a b : α} : down a = down b ↔ a = b := by sorry
...
@[target] theorem choose_spec (h : ∃ x, p x) : p (choose h) := by sorry

end FindA

/-- A constructive version of `Classical.axiom_of_choice` for `Encodable` types. -/
@[target] theorem axiom_of_choice {α : Type*} {β : α → Type*} {R : ∀ x, β x → Prop} [∀ a, Encodable (β a)]
    [∀ x y, Decidable (R x y)] (H : ∀ x, ∃ y, R x y) : ∃ f : ∀ a, β a, ∀ x, R x (f x) := by sorry

@[target] theorem skolem {α : Type*} {β : α → Type*} {P : ∀ x, β x → Prop} [∀ a, Encodable (β a)]
    [∀ x y, Decidable (P x y)] : (∀ x, ∃ y, P x y) ↔ ∃ f : ∀ a, β a, ∀ x, P x (f x) := by sorry
...
@[target] theorem rel_sequence {r : β → β → Prop} {f : α → β} (hf : Directed r f) (a : α) :
    r (f a) (f (hf.sequence f (encode a + 1))) := by sorry

section

variable (hf : Directed (· ≤ ·) f)

@[target] theorem sequence_mono : Monotone (f ∘ hf.sequence f) := by sorry

@[target] theorem le_sequence (a : α) : f a ≤ f (hf.sequence f (encode a + 1)) := by sorry
...
@[target] theorem sequence_anti : Antitone (f ∘ hf.sequence f) := by sorry
