...
@[target] theorem small_univ_iff : Small.{u} (@Set.univ α) ↔ Small.{u} α := by sorry
...
@[target] theorem small_sInter {s : Set (Set α)} {t : Set α} (ht : t ∈ s)
    [Small.{u} t] : Small.{u} (⋂₀ s) := by sorry
...
@[target] theorem small_biInter {s : Set ι} {i : ι} (hi : i ∈ s)
    (f : (i : ι) → i ∈ s → Set α) [Small.{u} (f i hi)] : Small.{u} (⋂ i, ⋂ hi, f i hi) := by sorry
...
@[target] theorem small_empty : Small.{u} (∅ : Set α) := by sorry

@[target] theorem small_single (x : α) : Small.{u} ({x} : Set α) := by sorry

@[target] theorem small_pair (x y : α) : Small.{u} ({x, y} : Set α) := by sorry
