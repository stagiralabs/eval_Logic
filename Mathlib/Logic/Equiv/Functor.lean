...
@[target] theorem mapEquiv_apply (h : α ≃ β) (x : f α) : (mapEquiv f h : f α ≃ f β) x = map h x := by sorry

@[target] theorem mapEquiv_symm_apply (h : α ≃ β) (y : f β) :
    (mapEquiv f h : f α ≃ f β).symm y = map h.symm y := by sorry

@[target] theorem mapEquiv_refl : mapEquiv f (Equiv.refl α) = Equiv.refl (f α) := by sorry
