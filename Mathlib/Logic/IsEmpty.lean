...
protected @[target] theorem Function.isEmpty [IsEmpty β] (f : α → β) : IsEmpty α := by sorry
...
@[target] theorem Subtype.isEmpty_of_false {p : α → Prop} (hp : ∀ a, ¬p a) : IsEmpty (Subtype p) := by sorry
...
@[target] theorem forall_iff {p : α → Prop} : (∀ a, p a) ↔ True := by sorry

@[target] theorem exists_iff {p : α → Prop} : (∃ a, p a) ↔ False := by sorry
...
@[target] theorem not_nonempty_iff : ¬Nonempty α ↔ IsEmpty α := by sorry

@[simp]
theorem not_isEmpty_iff : ¬IsEmpty α ↔ Nonempty α :=
  not_iff_comm.mp not_nonempty_iff

@[target] theorem isEmpty_Prop {p : Prop} : IsEmpty p ↔ ¬p := by sorry

@[target] theorem isEmpty_pi {π : α → Sort*} : IsEmpty (∀ a, π a) ↔ ∃ a, IsEmpty (π a) := by sorry

@[target] theorem isEmpty_fun : IsEmpty (α → β) ↔ Nonempty α ∧ IsEmpty β := by sorry

@[target] theorem nonempty_fun : Nonempty (α → β) ↔ IsEmpty α ∨ Nonempty β := by sorry

@[target] theorem isEmpty_sigma {α} {E : α → Type*} : IsEmpty (Sigma E) ↔ ∀ a, IsEmpty (E a) := by sorry

@[target] theorem isEmpty_psigma {α} {E : α → Sort*} : IsEmpty (PSigma E) ↔ ∀ a, IsEmpty (E a) := by sorry

theorem isEmpty_subtype (p : α → Prop) : IsEmpty (Subtype p) ↔ ∀ x, ¬p x := by
  simp only [← not_nonempty_iff, nonempty_subtype, not_exists]

@[target] theorem isEmpty_prod {α β : Type*} : IsEmpty (α × β) ↔ IsEmpty α ∨ IsEmpty β := by sorry
...
@[target] theorem isEmpty_psum {α β} : IsEmpty (α ⊕' β) ↔ IsEmpty α ∧ IsEmpty β := by sorry

@[target] theorem isEmpty_ulift {α} : IsEmpty (ULift α) ↔ IsEmpty α := by sorry

@[target] theorem isEmpty_plift {α} : IsEmpty (PLift α) ↔ IsEmpty α := by sorry

@[target] theorem wellFounded_of_isEmpty {α} [IsEmpty α] (r : α → α → Prop) : WellFounded r := by sorry

@[target] theorem isEmpty_or_nonempty : IsEmpty α ∨ Nonempty α := by sorry

@[target] theorem not_isEmpty_of_nonempty [h : Nonempty α] : ¬IsEmpty α := by sorry
...
@[target] theorem leftTotal_empty [IsEmpty α] : LeftTotal R := by sorry

@[target] theorem leftTotal_iff_isEmpty_left [IsEmpty β] : LeftTotal R ↔ IsEmpty α := by sorry

@[target] theorem rightTotal_empty [IsEmpty β] : RightTotal R := by sorry

@[target] theorem rightTotal_iff_isEmpty_right [IsEmpty α] : RightTotal R ↔ IsEmpty β := by sorry

@[target] theorem biTotal_empty [IsEmpty α] [IsEmpty β] : BiTotal R := by sorry

@[target] theorem biTotal_iff_isEmpty_right [IsEmpty α] : BiTotal R ↔ IsEmpty β := by sorry

@[target] theorem biTotal_iff_isEmpty_left [IsEmpty β] : BiTotal R ↔ IsEmpty α := by sorry
