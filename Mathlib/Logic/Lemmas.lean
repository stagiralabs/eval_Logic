...
@[target] theorem iff_assoc {a b c : Prop} : ((a ↔ b) ↔ c) ↔ (a ↔ (b ↔ c)) := by sorry
...
@[target] theorem ite_dite_distrib_right {a : q → α} {b : ¬q → α} {c : α} :
    ite p (dite q a b) c = dite q (fun hq ↦ ite p (a hq) c) fun hq ↦ ite p (b hq) c := by sorry

@[target] theorem dite_ite_distrib_left {a : p → α} {b : ¬p → α} {c : ¬p → α} :
    (dite p a fun hp ↦ ite q (b hp) (c hp)) = ite q (dite p a b) (dite p a c) := by sorry

@[target] theorem dite_ite_distrib_right {a : p → α} {b : p → α} {c : ¬p → α} :
    dite p (fun hp ↦ ite q (a hp) (b hp)) c = ite q (dite p a c) (dite p b c) := by sorry

@[target] theorem ite_ite_distrib_left : ite p a (ite q b c) = ite q (ite p a b) (ite p a c) := by sorry

@[target] theorem ite_ite_distrib_right : ite p (ite q a b) c = ite q (ite p a c) (ite p b c) := by sorry

@[target] lemma Prop.forall {f : Prop → Prop} : (∀ p, f p) ↔ f True ∧ f False := by sorry
